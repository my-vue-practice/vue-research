...
// 匹配"<"开头后面紧跟命名的。（dom开始标签）
var startTagOpen = new RegExp("^<" + qnameCapture);
// 匹配空标签结尾（如：<br />中的"/>"）
var startTagClose = /^\s*(\/?)>/;
// [^>] 和 [>]意思相反，一个是不包含，一个是包含。 "^": 反向字符集
// 匹配"</"开头和">"结尾的标签（dom结束标签）
...
// 将html特殊符号进行解码操作。shouldDecodeNewlines 是否需要解码\n \t
...
//     // isPlainTextElement判断是否为文本内容元素，比如 script,style,textarea
...
//         // 条件注释，一般用于处理IE兼容，如 <!--[if IE]> xxx <![endif]-->
//         // 这里只处理了<![if !IE]> xxx <![endif]> 这种情况的，不知为啥。
...
function parseHTML(html, options) {
  ...
  while (html) {
    ...
    // Make sure we're not in a plaintext content element like script/style
    // isPlainTextElement判断是否为文本内容元素，比如 script,style,textarea
    // 栈顶元素lastTag不存在 或 栈顶元素不为script,style,textarea三个中的一个。
    if (!lastTag || !isPlainTextElement(lastTag)) {
      // 记录第一个标签“<”的位置（标记为文本结尾位置，后面会再次检查是否是真的文本结尾位置）
      var textEnd = html.indexOf("<");
      if (textEnd === 0) {
        // Comment: 注释处理
        if (comment.test(html)) {
        ...
        // Start tag:
        var startTagMatch = parseStartTag();
        // 匹配到标签开始，就进行入栈处理（handleStartTag）
        if (startTagMatch) {
          handleStartTag(startTagMatch);
          if (shouldIgnoreFirstNewline(startTagMatch.tagName, html)) {
            advance(1);
      ...
      if (textEnd >= 0) {
        // 从下一个标签“<”开始的剩余部分
        rest = html.slice(textEnd); // 以“<”开头的字符串，不一定就是标签，也有可能是文本中存在"<"。这个时候需要将其作为文本处理。
        while (
          !endTag.test(rest) && // 非结束标签
          !startTagOpen.test(rest) && // 非开始标签
          !comment.test(rest) && // 非注释
          !conditionalComment.test(rest) // 非条件注释
        ) {
          // < in plain text, be forgiving and treat it as text
          // 存在“<”符号，当文本处理（模板中存在误写的“4<3”，此时textEnd的位置在文本中，位置不正确，需要调整到正确的文本内容结尾位置）
          next = rest.indexOf("<", 1);
          if (next < 0) {
            break;
          } // 代表后面不存在"<"了，就都当做文本内容处理。
          // 更新文本内容结尾索引
          textEnd += next;
          // 更新剩余字符串，再次进入while判断，知道找到以上面4个规则（开始标签、结束标签、注释标签、条件注释标签）开头的为止
          rest = html.slice(textEnd);
        }
        // 获取文本内容
        text = html.substring(0, textEnd);
      ...
      // textEnd < 0 说明剩余的字符模板中不存在"<"了，测试全部作为文本内容处理。
      if (textEnd < 0) {
        text = html;
      }
      // 获取到文本内容之后，继续调整模板字符串（将处理过的text截掉）
      if (text) {
        advance(text.length);
      }
    } else {
      // 栈顶元素lastTag为script,style,textarea三个中的一个
      var endTagLength = 0;
      var stackedTag = lastTag.toLowerCase();
      var reStackedTag =
        reCache[stackedTag] ||
    ...
    // 经过上面一番处理完之后，发现html模板字符串没变，也就是没处理，说明模板字符串格式不对。
    if (html === last) {
      if (
  ...
  // 指针移动，不断的将模板内容html进行往后截取。知道最终处理完成。
  function advance(n) {
    index += n;
    html = html.substring(n);
  }
  ...
  */
  function parseStartTag() {
    ...
    if (start) {
      ...
      while (
        // 非标签结尾
        !(end = html.match(startTagClose)) && // 匹配自闭合标签结尾（如：<br />中的"/>"）
        // 匹配属性（动态、静态属性）
        (attr =
          html.match(dynamicArgAttribute) || // 动态属性匹配（v-xxx、@、:、#）
  ...
  // 对match数据进行优化处理，得到最终数据格式存入stack栈中
  // 入栈处理
  function handleStartTag(match) {
    var tagName = match.tagName;
    var unarySlash = match.unarySlash;
...
function parseComponent(content, options) {
  if (options === void 0) options = {}; // void 0 === undefined
...
/*  */
// 解析模板时和平台相关的一些配置（这里是web平台）
var baseOptions = {
  expectHTML: true, // 期望是html，作用不明
  modules: modules, //
  ...
  canBeLeftOpenTag: canBeLeftOpenTag, // 可以不闭合的标签，比如tr、td等。
...
var compile = ref.compile; // 返回 { ast, render, staticRenderFns, errors, tips}
